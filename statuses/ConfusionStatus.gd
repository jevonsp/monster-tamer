...
func _calculate_self_hit_damage(owner: Monster) -> int:
	var efficacy := TypeChart.get_attacking_type_efficacy(TypeChart.Type.NONE, owner)
	...
	var attacking_multi = owner.stat_stages_and_multis.stat_multipliers[Monster.Stat.ATTACK]
	...
	var defending_multi = owner.stat_stages_and_multis.stat_multipliers[Monster.Stat.DEFENSE]
